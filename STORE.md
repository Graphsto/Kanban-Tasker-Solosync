...
4. Download the `store-submission-<product-version>` Actions artifact. It contains the x64/ARM64 MSIX bundle, `SHA256SUMS.txt`, `build-report.json`, `verification.json`, packaging logs and architecture-specific license/binary inventories. Keep an archival copy outside Actions before its 30-day retention expires.
...
A repeated preparation for the same commit reuses an unchanged, asset-free draft and leaves its original artifact intact. A failed build can be rerun. If the artifact has expired, or the source/version mapping changed, investigate before preparing another version; the workflow does not silently replace evidence or reuse a published tag. An existing release/tag, failed or stale checks, incomplete package evidence, unexpected profile or a checksum mismatch stops preparation.
