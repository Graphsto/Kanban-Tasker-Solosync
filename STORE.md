...
4. Download the `store-submission-<product-version>-<run>-<attempt>` Actions artifact. It contains the x64/ARM64 MSIX bundle, `SHA256SUMS.txt`, `build-report.json`, `verification.json`, packaging logs and architecture-specific license/binary inventories. Keep an archival copy outside Actions before its 30-day retention expires.
...
A repeated preparation for the same commit reuses an unchanged, asset-free draft and leaves its original artifact intact. GitHub does not expose private drafts to the read-only build token, so a retry can rebuild; each run/attempt has a unique artifact name to prevent collisions. The write-enabled draft job sees all drafts and makes the final version/commit check without changing an existing draft. A failed build can be rerun. If the artifact has expired, or the source/version mapping changed, investigate before preparing another version; the workflow does not silently replace evidence or reuse a published tag. An occupied version/tag, failed or stale checks, incomplete package evidence, unexpected profile or a checksum mismatch stops preparation.
